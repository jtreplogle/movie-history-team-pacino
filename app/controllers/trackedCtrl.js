...
MovieApp.controller("trackedCtrl", [
  "$scope",
  "$http",
  "$compile",
  "firebaseURL",
  function ($scope,$http,$compile,firebaseURL) {
    console.log("RUNNING TRACKEDCTRL");
    $http.get(`${firebaseURL}/movies/.json`)
    .then((response) => {
      $scope.omnibusArr = processFirebase(response)
      $scope.omnibusArr = $scope.omnibusArr.filter(filterTracked);
    ...
    function filterTracked(obj) {
      return obj.tracking === true;
    }
